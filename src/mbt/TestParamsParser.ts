...
export default class TestParamsParser {
  public static parseTestData(testData: string): Map<number, TestData> {
    logger.debug(`parseTestData: testData="${testData}"`);
    ...
    arrTestParam.forEach(p => {
      try {
        ...
        const parsedTestData = new MbtTestParser().parseTestParam(testParts);
